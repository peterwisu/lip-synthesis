...
class SyncNet(nn.Module):
    def __init__(self,bilstm=True):
        ...
        self.pe = PositionalEncoding(d_model=256, dropout=0.1, max_len=5)
        ...
        self.pool = GlobalAvgPooling(self.lip_size)
    ...
    def forward(self, audio, lip):
        ...
        attn_out = self.self_attn(vis_hidden, vis_hidden,vis_hidden)[0]

        pool_out = self.pool(attn_out).squeeze(1)
        ...
        #print(vis_hidden.shape)
        #exit()
        # embeddings
        lip = self.lip_fc(pool_out)
        au = self.audio_fc(au)
...
class GlobalAvgPooling(nn.Module):
    ...
    def __init__(self,n_dim):
        ...
        self.avg_pool = nn.AdaptiveAvgPool2d((1, n_dim))
